...
`P(成为候选) = 1-(1-J^r)^b` 即成为候选的概率 $P = 1-(1-J^r)^b$，并给定召回目标反推最小 bands。
...
不是 bug，是设计。LSH 分带的概率 $P = 1-(1-J^r)^b$ 对中等相似度（0.4-0.6）
并不接近 0——查教程 S 曲线表：$J=0.45$ 在 $16\times4$ 配置下候选概率高达 48.8%，doc03/dupC
的 64 维签名采样一致率又是 0.53，撞上一个完全相同的 band 是**近半概率的常态事件**。
LSH 的哲学是**用高召回换误报**（宁可多报、绝不漏报：真重复 2 对零漏召），
...
**Q4：** FineWeb 配置 14 band × 8 row 的"等效阈值 ≈ 0.72"是怎么来的？如果把 rows
从 8 改成 4（保持签名维度 112，即 28 band × 4 row），管线的召回-误报会怎么移动？

<details>
<summary>💡 答案</summary>

S 曲线拐点近似 $(1/b)^{1/r}$：$(1/14)^{1/8} \approx 0.72$——相似度
高于它的对大概率成候选、低于的大概率被过滤（论文表述为"目标相似度 ≥75%"）。改成 28×4 后
拐点 $\approx (1/28)^{1/4} \approx 0.44$：
...
- **结论**：分带 LSH 对高相似对是"必然命中"，漏检概率随 J 增大指数归零。
- **原理**：候选概率 $P = 1-(1-J^r)^b$——J=0.9、16×4 配置下漏检仅 $3.8\times10^{-8}$；低相似对也可能撞进候选（J=0.45 时近一半），靠第④步 Jaccard 精算兜底。
- **边界**：代价是候选误报（"宽进严出"），验证步计算量取决于候选数——b 调大召回升、验证成本同步升。

**Q2："去重放在质量过滤前还是后？"**

- **结论**：两流派都有消融背书，答出"先去重省算力、先过滤保精度"的分野即可。
- **原理**：先去重——重复文档会重复消耗质量过滤的算力；先过滤——低质文档先删能减少假重复（短文本空集判重那类事故，见本章错误 2）。
- **边界**：取决于语料污染程度；给出工程经验值"先测重复率，高污染先去重"即可（02 章练习 Q1 有两派完整论据）。

**Q3："FineWeb 的去重配置？"**

- **结论**：5-gram、112 哈希 = 14 band × 8 row，目标相似度 ≥75%（arXiv:2406.17557 §3.4）。
- **原理**：等效阈值 $J^* \approx (1/14)^{1/8} \approx 0.72$；s=0.75 命中 77%、s=0.85 命中 98.8%（附录 D.1）——S 曲线的两根钉子。
- **边界**：配置随语料与算力消融，无普适值（本课玩具版 16×4 对应阈值 0.5）；引用时带出处，别背无源数字。

**Q4："字面重复 vs 语义重复？"**

- **结论**：MinHash 管字面重复；语义重复要 embedding，成本高一个量级。
- **原理**：字面重复在 shingle 集合层面就有高 Jaccard；重改写的"语义重复"字面信号弱（本课 dupB J≈0.24，连候选都不是），只有语义向量近邻能抓到。
- **边界**：语义去重的召回依赖 embedding 对领域的泛化，且向量检索要另建索引——先算字面账，再决定要不要上语义档。

**Q5："簇消解保留哪一篇？"**

- **结论**：三种策略——keep-first / keep-longest / 带质量分的 keep-best，工业默认是前两种之一。
- **原理**：重复关系是等价关系，须先连通成簇（并查集）再选择；keep-first 稳定可复现，keep-longest 假设"更长=更完整"。
- **边界**：keep-longest 会被"注水长文"钻空子；生产上带质量分的 keep-best 最优，但质量分本身是个模型（🌟 题 5 就是它的简化版）。
