...
- **结论**：拼接式是当下主流，Flamingo 门控是历史方案，early-fusion 是下一代原生多模态的方向。
- **原理**：拼接式（LLaVA）把投影器输出的视觉 token 直接拼进 LLM 序列——脚本 01 实测的关键性质是 LLM 本体不感知模态差异，图像位置直接拿投影向量当输入 embedding；Flamingo 用门控交叉注意力把视觉注入冻结 LLM，结构重、已淡出主流；early-fusion（Fuyu/Chameleon 式）从预训练起就让图文共享一条管线，对齐不是"后补"而是"原生"。
- **边界**：拼接式受制于视觉塔与投影器的对齐质量；early-fusion 训练成本高，基本是基座厂商的玩法。

**Q2："LLaVA 两阶段为什么这么设计？"**

- **结论**：先冻结 ViT 和 LLM 只训投影器，把"翻译器"搭稳后再端到端——防止随机初始化的噪声 token 冲垮预训练能力。
- **原理**：脚本 01 的实测参数账：Stage 1 只有 1,856 个可训练参数（loss 2.907→1.825），Stage 2 全解冻 29,620 参数（loss→0.034）；真实 LLaVA-7B 同构投影器（题 3 的 `mlp2x_params(1024, 4096)`）也只有 20,979,712 ≈ 21M，约占全模型 0.3%——先训这座小桥几乎不动两个基座。
- **边界**：一开始就端到端，三个模块同时漂移，损失下降可能只是"互相迁就"而非对齐变好；代价是多一轮训练流程。

**Q3："CLIP vs SigLIP？"**

- **结论**：InfoNCE 是全 batch 的 softmax 对比（batch 依赖强），SigLIP 拆成逐对独立 sigmoid（batch 依赖弱）——小 batch 场景 SigLIP 是更稳的默认。
- **原理**：InfoNCE 的负样本来自 batch 内其他文本，CLIP 论文为此用 32768 的 batch；SigLIP 对每对独立做 $+1/-1$ 的二分类，论文实测 batch 缩到 1/4 仍持平，这正是 SmolVLM/InternVL/PaliGemma 改用它的原因（省显存）；脚本 02 实测训练后学到的温度 $\tau$：CLIP 路径 16.21、SigLIP 8.53——锐度是学出来的，不是定的。
- **边界**：$\tau$ 经 `scale = exp(log_scale)` 可学习保证恒正、自适应锐度，但初始化不当（过小信号弱、过大早期不稳）仍会翻车。

**Q4："Qwen-VL 为什么 OCR 强？"**

- **结论**：原生动态分辨率——token 数随图像内容自适应，不像固定 336² 那样在进模型前就把字糊掉。
- **原理**：固定 336² 对一张 4000×3000 的文档照片意味着下采样 100 倍以上，笔画在 resize 阶段就丢了，后端再强也救不回；动态分辨率按原始尺寸切 patch（`ceil(H/p) × ceil(W/p)`，题 1 实测 1024×768 → 4070 token），再配 pixel shuffle 类压缩（÷4，题 4 实测 4070//4 = 1017 token）与 token 预算控制（`max_tokens=2560` 触发逐级缩放）。
- **边界**：token 数不可预测，一张长图能吃爆上下文——预算与压缩工程是动态分辨率的必配代价，不是可选项。
