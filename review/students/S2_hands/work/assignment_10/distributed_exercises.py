...
def model_state_bytes(param_count, world_size, stage):
    ...
    Returns:
        int/float：字节数
    """
    # 按 stage 分支返回；未知 stage 抛 ValueError
    if stage == 'ddp':
        return 16 * param_count
    if stage == 'zero1':
        return 4 * param_count + 12 * param_count / world_size
    if stage == 'zero2':
        return 2 * param_count + 14 * param_count / world_size
    if stage == 'zero3':
        return 16 * param_count / world_size
    raise ValueError(f"未知 stage: {stage!r}（可选 'ddp'|'zero1'|'zero2'|'zero3'）")
...
def sampler_indices(n, world_size, rank, seed=0, with_torch=None):
    ...
    padded = base + base[:total - n]          # 从开头循环补齐
...
def sampler_coverage_ok(n, world_size, seed=0):
    ...
    Returns:
        bool
    """
    shards = [sampler_indices(n, world_size, r, seed=seed) for r in range(world_size)]
    if len({len(s) for s in shards}) != 1:    # 性质 1：长度相等
        return False
    union = set()
    for s in shards:
        if len(set(s)) != len(s):             # 性质 3：rank 内部无重复
            return False
        union.update(s)
    return union == set(range(n))             # 性质 2：全覆盖 0..n-1
...
def tp_mlp_max_error(X, W1, W2, n_shards=2):
    ...
    dense = gelu(X @ W1.T) @ W2.T
    h4 = W1.shape[0]
    chunk = h4 // n_shards
    y_sharded = 0
    for r in range(n_shards):
        w1_r = W1[r * chunk:(r + 1) * chunk, :]   # W1 按行（输出维）切
        w2_r = W2[:, r * chunk:(r + 1) * chunk]   # W2 按列（输入维）切
        y_sharded = y_sharded + gelu(X @ w1_r.T) @ w2_r.T
    return (y_sharded - dense).abs().max().item()
...
def in_flight_activations(schedule, m, p):
    ...
    if schedule == '1f1b':
        return p
    raise ValueError(f"未知调度: {schedule!r}（可选 'gpipe'|'1f1b'）")
