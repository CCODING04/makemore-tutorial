...
def throughput_tokens_per_s(n_requests, n_out_tokens, wall_seconds):
    ...
    """
    # TODO 已完成：总 token = 请求数 × 每请求输出，除以墙钟
...
def kv_cache_gb(n_layers, n_kv_heads, head_dim, seq_len, batch, bytes_per_elem=2):
    ...
    """
    # TODO 已完成：2(K+V) × layers × kv_heads × head_dim × seq × batch × bytes / 1e9
...
def max_batch_for_vram(kv_gb_per_seq, vram_gb=24.0, model_gb=4.0, headroom_gb=2.0):
    ...
    """
    # TODO 已完成：floor(可用显存 / 每序列 KV)，max(1, ...) 保底
    usable = vram_gb - model_gb - headroom_gb
    return max(1, int(usable // kv_gb_per_seq))
...
def static_batch_waste(jobs, pad_to_max=True):
    ...
    """
    # TODO 已完成：浪费率 = 1 − 实际/分配
    actual = sum(jobs)
    if pad_to_max:
        allocated = len(jobs) * max(jobs)
    else:
        allocated = actual
    return 1.0 - actual / allocated
...
def spec_decode_speedup(alpha, gamma, draft_overhead=0.0):
    ...
    """
    # TODO 已完成：产出 / 成本（成本 = 1 次 target 前向 + draft 折算开销）
    return spec_tokens_per_cycle(alpha, gamma) / (1.0 + draft_overhead)
...
def simulate_batching(arrivals, gen_lens, max_batch=8, mode="continuous"):
    ...
    """
    # TODO 已实现：continuous 逐步换人；static 整批陪跑
    n = len(arrivals)
    ...
    if mode == "continuous":
        ...
        while i < n or running:
            # 每步先把已到达且有槽位的请求装入
            while i < n and arrivals[i] <= t and len(running) < max_batch:
                running.append(gen_lens[i])
                i += 1
            if not running:        # GPU 全空：时钟跳到下一个到达
                t = max(t, arrivals[i])
                continue
            # 全体在跑各出 1 token
            slot_steps += len(running)
            tokens += len(running)
            running = [g - 1 for g in running if g - 1 > 0]
            t += 1
        makespan = t
    elif mode == "static":
        t = 0
        ...
        slot_steps = 0
        while i < n:
            # 从当前时刻已到达的请求里凑最多 max_batch 个
            start_idx = i
            batch = []
            while i < n and len(batch) < max_batch and arrivals[i] <= t:
                batch.append(gen_lens[i])
                i += 1
            if not batch:          # 等下一位到达
                t = arrivals[i]
                continue
            start = max(t, arrivals[start_idx])
            dur = max(batch)
            slot_steps += len(batch) * dur
            tokens += sum(batch)
            t = start + dur
        makespan = t
    else:
    ...
    return {"makespan": makespan,
            ...
            "tokens": tokens,
            "waste_rate": 1.0 - tokens / slot_steps}
