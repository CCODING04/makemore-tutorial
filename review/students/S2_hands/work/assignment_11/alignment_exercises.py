...
def group_advantages(rewards, eps=1e-6):
    ...
    std = max(var ** 0.5, eps)  # eps 在 max 里，不是 std + eps
...
def k3_kl(logp_ref, logp_new):
    ...
    for lr, ln in zip(logp_ref, logp_new):
        d = lr - ln
        kl += math.exp(d) - d - 1
    return kl / len(logp_ref)  # 取平均，不是返回累加和
...
def kl_budget_ok(logp_ref, logp_new, budget=0.05):
    ...
    常见陷阱：
        ...
        - 通常在 0.01-0.1 之间
    """
    kl = k3_kl(logp_ref, logp_new)
    return bool(kl <= budget)  # 边界：kl == budget 算在预算内
...
def zero_gradient_groups(reward_matrix):
    ...
    """
    result = []
    for i, group in enumerate(reward_matrix):
        if max(group) == min(group):  # 全同奖励 → 优势全零
            result.append(i)
    return result
