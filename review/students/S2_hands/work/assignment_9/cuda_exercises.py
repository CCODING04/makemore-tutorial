...
# 题 5 需要 triton（torch 自带）；无 GPU 环境下 import 也可能失败，稳妥起见 try 包住
try:
    import triton
...
def gflops_report(time_ms, M, N, K, peak_gflops=None):
    ...
    pct_of_peak = None if peak_gflops is None else gflops / peak_gflops * 100
    return {'flops': flops, 'gflops': gflops, 'pct_of_peak': pct_of_peak}
...
# ⚠️ @triton.jit 内核必须定义在模块顶层（嵌套定义会 NameError: tl is not defined）
if triton is not None:

    @triton.jit
    def _softmax_kernel(output_ptr, input_ptr,
        ...
        row_minus_max = row - tl.max(row, axis=0)   # 减最大值防上溢
        numerator = tl.exp(row_minus_max)
        softmax_output = numerator / tl.sum(numerator, axis=0)
        tl.store(out_row_start_ptr + col_offsets, softmax_output,
...
def triton_softmax(x):
    ...
    _softmax_kernel[(n_rows,)](output, x, x.stride(0), output.stride(0),
