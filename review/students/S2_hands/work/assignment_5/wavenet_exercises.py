...
def build_wavenet(vocab_size, n_embd=10, n_hidden=68, block_size=8):
    ...
    model = Sequential([
        ...
        # ---- 层 3 ----
        FlattenConsecutive(2), Linear(n_hidden * 2, n_hidden, bias=False), BatchNorm1d3D(n_hidden), Tanh(),
        # ---- 输出层：末段形状 (B, 1, n_hidden) → 展平 → (B, vocab_size) ----
        Flatten(),
...
def verify_shapes(model, vocab_size=27, block_size=8, batch_size=4):
    ...
    for layer in model.layers:
        x = layer(x)
        shapes.append((layer.__class__.__name__, tuple(x.shape)))
    assert shapes[-1][1] == (batch_size, vocab_size), (
        f"最终 shape 应为 {(batch_size, vocab_size)}，得到 {shapes[-1][1]}"
    )
    return shapes
...
def train_wavenet(words, n_embd=24, n_hidden=128, block_size=8,
    ...
    for i in range(steps):
        ...
        lr = 0.1 if i < int(steps * 0.6) else (0.05 if i < int(steps * 0.8) else 0.01)
        for p in model.parameters():
            p.data += -lr * p.grad
    ...
    with torch.no_grad():
        logits = model(Xdev)
        dev_loss = F.cross_entropy(logits.view(-1, logits.shape[-1]), Ydev)
