...
def test_train_step():
    ...
    n_embd, n_hidden, block_size = 10, 100, 3
    # 初始化用小尺度（Part 3 的 Kaiming 之前，先手动缩小）：
    # std=1 的 W1/W2 会让初始 logits 过大（实测 CE≈18），任何正确实现都过不了 <10 的检查；
    # 缩放后初始 CE≈3.37 ≈ ln(27)=3.296，与下方注释的预期一致。
    C = torch.randn(27, n_embd, requires_grad=True)
    W1 = (torch.randn(block_size * n_embd, n_hidden) * 0.1).requires_grad_(True)
    b1 = (torch.randn(n_hidden) * 0.01).requires_grad_(True)
    W2 = (torch.randn(n_hidden, 27) * 0.1).requires_grad_(True)
    b2 = (torch.randn(27) * 0.01).requires_grad_(True)
