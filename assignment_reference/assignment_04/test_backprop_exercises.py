...
def test_q4_batchnorm_backward():
    ...
    # ⚠️ 修复：参数必须是带梯度的叶子，否则 loss.backward() 会报
    # "does not require grad"（原测试 bug：任何正确实现都无法通过）
    ...
    # 阈值 1e-4：dhprebn_ref 与被测函数走的是两条独立的 float32 计算链
    # （ref 用重建的图），n=32 的 BN 反传下 1e-5 的 max 阈值会因数值噪声误报
    # （实测正确实现 max diff ≈ 4.8e-05）
    ok = diff < 1e-4
    print(f"  {'✅' if ok else '❌'} max diff = {diff:.2e}, mean diff = {mean_diff:.2e}")
