...
- **结论**：三个部件白板直接写——IDF 恒正平滑、TF 饱和、文档长度归一（课程与作业同款 $k_1=1.2,\ b=0.75$）。
- **原理**：$\mathrm{score}(q,d)=\sum_{q}\mathrm{idf}(q)\cdot\frac{tf\,(k_1+1)}{tf+k_1\,(1-b+b\,|d|/\mathrm{avgdl})}$，IDF 用恒正平滑式 $\ln\frac{N-df+0.5}{df+0.5}+1$；TF 饱和保证单调但不线性（题 2 验收：出现 3 次的 chunk 得分高于出现 1 次的），IDF 恒正保证查询词不在任何 chunk 时全 0 分、稀有词最高分远超常见词。
- **边界**：BM25 输出无界正分（一个稀有词命中就能到 5+），与 cosine 融合前必须归一化；纯词面匹配对同义改写无能为力。

**Q2："hybrid 搜索两路分数怎么融合？"**

- **结论**：默认答 RRF——只融合名次不融合分值、免标定（$k=60$ 默认）；追问再讲加权 + min-max 归一化 + 网格搜，并用"融合不是免费"的实测收尾。
- **原理**：$\mathrm{score}(i)=\sum_{L}1/(k+\mathrm{rank}_L(i))$，$k\to\infty$ 时退化为计数排序（k 控制名次信息的带宽，题 3 的退化性质）；要加权就必须两路各自 min-max 归一化，否则 BM25 的无界正分统治 cosine（题 5）；Anthropic contextual retrieval 的官方四级失败率链 5.7%→3.7%→2.9%→1.9%（+上下文 → +hybrid → +rerank，累计 −67%）是混合价值的最好背书——但本课 01 章实测 Q2 上 BM25 单路 1.00 → hybrid 0.80，融合也会倒退。
- **边界**：RRF 稳健但不是最优（01 章四路 mean：dense 0.58 / BM25 0.60 / hybrid 0.85 / +rerank 0.92）；最优 w 依赖数据，要按下游 recall 网格搜出来。

**Q3："RAG 怎么评测？"**

- **结论**：检索侧 recall@k（必须有 ground truth），生成侧 RAGAS 四指标——重点讲 faithfulness 的 claims 分解与评测器噪声。
- **原理**：faithfulness 把答案拆成逐条 claim 让裁判逐条判支持（yes=1，no/unsure=0 的保守口径，宁可低估不虚高；本课题 4 的 judge 可注入正是为了让 mock 离线可测）；评测器本身有噪声——等价问法就能让小裁判翻转，所以要 supported/refuted/unknown 三数齐报，别让一个标量压扁三种状态；检索侧 recall@k 直接可视化组件贡献：本课降级实测（`RAG18_FORCE_FALLBACK=1`，hashing trick 换掉真嵌入）dense 列 recall 0.58→0.20。
- **边界**：没有 ground truth 时 recall@k 不可算；裁判模型换一代分数口径就变——报告必须附裁判版本与运行配置。

**Q4："chunk 大小怎么定？"**

- **结论**：没有银弹——256-1024 字符起步、按下游 recall 调；碎块丢上下文的解法是 overlap、contextual retrieval（先切后补）或 late chunking（先嵌后切）。
- **原理**：块太大检索粒度糊、太小上下文残缺——所以先守住三条不变量（长度受限、overlap 接头、不丢字符，本课题 1 的验收）；contextual retrieval 给每块补文档级定位句，通用但每块一次 LLM 调用，late chunking 省 token 但绑定长上下文嵌入模型；工程坑：`s.split('。')` 会丢句号，要用捕获组 `re.split('(。)', s)` 把句号保留成独立原子段。
- **边界**：参数要真的被用上并两组复现（`size=512/overlap=64` 与 `size=128/overlap=16` 都过不变量），"一版参数到处用"不成立。

**Q5："什么时候不用 RAG？"**

- **结论**：先算量级——装得下上下文就直塞；知识要引用/常更新才上 RAG；要改模型行为才微调。
- **原理**：三关判定——更新频率（文档天天变，增量重嵌入比重发全量 prompt 便宜）、多租户/权限（按查询检索比把所有文档塞进共享上下文安全）、成本规模（Self-Route，arXiv 2407.16833，实测 RAG 的 token 消耗仅约为长上下文直塞的 1/6）；本课思考题 Q4 的例子：50k token 文档对 128k 上下文模型，首推直接塞 prompt。
- **边界**：微调改变的是风格/能力而非知识新鲜度（LaRA，arXiv 2502.09977：两者各有所长、无银弹）——拿微调解决知识问题答非所问。
