...
- 把 `WHITELIST` 里加上 `echo`，重跑 Demo 4：模型的 echo+命令替换读文件链路会
...
**Q1："agent loop 的核心是什么？"**

- **结论**：一个 while 循环——模型输出 → 解析工具调用 → 执行 → 结果回填上下文 → 继续，直到触发三种终止条件之一；剩下全是工程加固。
- **原理**：终止三条件有优先级（本课题 3：max_turns > loop_detected > no_tool_calls）——模型不再调工具（给最终答案）、轮数上限（防失控）、同工具连续 3 次（防循环）；加固件是解析兜底（畸形输出返回 `[]` 绝不抛异常，教程 01 章陷阱 4 的"最小化影响半径"）、沙箱与上下文裁剪。
- **边界**：宽口径循环检测（只看工具名）会误伤"连读 3 个不同文件"的正常探索，严口径（同名同参）拦不住参数微变的复读——更稳的是看状态是否在演进。

**Q2："为什么 bash 工具要白名单+超时？"**

- **结论**：模型输出是不可信输入——白名单防破坏性命令、超时防挂死、截断防上下文爆炸，安全边界必须放在执行侧。
- **原理**：白名单只看命令第一个词（`{"ls","cat","grep","python"}`）；课程实测教训：教程 01 章 Demo 4 里 0.5B 模型真的绕开专用工具，用 echo 加命令替换去读 `/tmp/agent_demo2.txt` 再回显，被白名单拦截（`command 'echo' not allowed`）——把 echo 加进白名单就等于给"绕开专用工具"开门，这正是本 Part 实验题的验证点；连 description 里写明白名单也只能"主要"约束住小模型。
- **边界**：白名单只是第一道门——生产 agent 还要只读文件系统、命令审计日志与环境终态验收，提示词约束可被注入绕过。

**Q3："MCP 是什么？和 A2A 什么关系？"**

- **结论**：MCP 管 agent↔工具，A2A 管 agent↔agent（Agent Card 发现），AGENTS.md 管 agent↔代码库——三层各管一段、不互替。
- **原理**：MCP 是 JSON-RPC 三步握手（本课题 5 的 mock transport）：带 id 的 `initialize` 请求 → 无 id 的 `notifications/initialized` 通知 → 新 id 的 `tools/call`；"通知无 id、不得回响应"正是课程脚本 02 第一版真实 bug 的来源；A2A 解决跨厂商 agent 互发现与协作。
- **边界**：目前不存在"用一个协议统一一切"——选型按通信对象分层，混着用是常态。

**Q4："subagent 的收益来自数量还是上下文隔离？"**

- **结论**：来自上下文隔离，不是数量——子任务可自包含才值得分兵。
- **原理**：Anthropic 深度研究系统的多 agent 架构用约 15× token，买到的是"每个 subagent 一份干净上下文"（并行检索类任务的标准答案）；反方 Cognition《Don't Build Multi-Agents》指出隔离的传递是有损压缩——子任务要反复回问主上下文时分兵只会放大损耗；判据是任务可分解性，不是"更多 agent = 更多智能"。
- **边界**：15× 是系统级乘数不是常数项，token 成本随分兵数量相乘——预算敏感与强世界依赖（并行行动会互相改状态）的场景慎用。

**Q5："怎么评 agent？"**

- **结论**：环境终态判分（不信话术）+ 政策合规 + pass^1/pass^k；报数字必须锁基准版本与脚手架口径。
- **原理**：τ-mini 实测（Qwen2.5-0.5B）里 T2 的模型"嘴上说抱歉、手上把地址改了"——合规评测的对象是行为不是话术，所以判分查 DB 终态；诚实登记：一次完整运行三个任务全部 runs=[False, False, False]、OVERALL pass^1=0.00，另一次 0.11——9 个样本的置信区间宽到没有统计意义，小样本评测必须附运行次数与温度；τ-bench 上游修过评分 bug 后部分模型榜单随之变化——评测基准也是代码。
- **边界**：pass^k 越大越苛刻（k 次全过才算过，pass^8 通常远低于 pass^1），跨榜比较必须同 commit hash + 模型版本 + 采样参数，否则没有工程含义。
