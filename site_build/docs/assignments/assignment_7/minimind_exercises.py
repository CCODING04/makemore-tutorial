...
class RMSNorm(nn.Module):
    ...
    def __init__(self, dim, eps=1e-6):
        super().__init__()
        self.eps = eps
        # TODO: 定义 self.weight = nn.Parameter(torch.ones(dim))
        pass
    ...
    def forward(self, x):
        ...
        # TODO: 实现 RMSNorm 前向
        # 提示:
        #   1. rms = x.pow(2).mean(-1, keepdim=True).add(self.eps).sqrt()
...
class SwiGLU(nn.Module):
    ...
    def forward(self, x):
        """输入 x: (..., dim)，输出 (..., dim)。"""
        # TODO: 实现 SwiGLU 前向
        # 提示: F.silu(self.gate_proj(x)) * self.up_proj(x) -> self.down_proj(...)
        pass
        return None
