...
**Q1："为什么 scaled attention 要除以 √d_k？不除会怎样？"**

- **结论**：除以 $\sqrt{d_k}$ 是为了把 $q \cdot k$ 的方差从 $\approx d_k$ 压回 $\approx 1$，防止 softmax 初始化时就坍缩成 one-hot。
- **原理**：若 q、k 各分量是零均值单位方差的独立项，内积是 $d_k$ 个独立项之和，方差 $\mathrm{Var}(q \cdot k) \approx d_k$，值随维度增大越来越尖锐；softmax 对大值敏感，会把每行推向 one-hot，初始化时每个 token 只聚合一个 token，通信失效。课程实测（作业 6 题 4 的属性测试）：未缩放时 `std ≈ sqrt(head_size)`，缩放后 `std ≈ 1`。
- **边界**：这是"初始化时保持注意力扩散"的论证，训练中学到的权重分布不受此约束；head_size 越小，不缩放的恶果越不明显。

**Q2："causal mask 有几种实现方式？你写过哪几种？"**

- **结论**：三种数学等价的写法——for 循环加权平均、tril 矩阵乘法、`masked_fill(-inf)` + softmax，工业标准是第三种。
- **原理**：课程脚本 03 用三种方式实现"过去 token 的加权聚合"并验证等价：v1 逐位置做前缀平均、v2 用下三角矩阵乘法一次算完（两者都是固定权重的平均）、v3 把上三角置 $-\infty$ 再过 softmax（softmax 后未来权重恰为 0）。只有 v3 的权重是数据依赖的（$q k^T$ 亲和力），所以真正的 attention 用 v3。
- **边界**：v1 无法并行，v2/v3 都要物化 $(T, T)$ 权重矩阵；三种写法在课程里只是教学阶梯，v1/v2 不含可学习的通信。

**Q3："Pre-LN 和 Post-LN 有什么区别？为什么现代 Transformer 都用 Pre-LN？"**

- **结论**：Pre-LN 把 LayerNorm 放进残差支路（`x = x + sublayer(LN(x))`），残差主干干净直达输入，深层训练更稳，是 GPT-2 起的现代标准；原始论文是 Post-LN。
- **原理**：教程 03 章明确"论文原版是 post-norm，现在更常见的是 pre-norm，本课采用 pre-norm"；课程实测 loss 演进（原视频数字）：+残差 2.08 → +LayerNorm 2.06，GPU 完整超参最终 1.48——残差让梯度"走高速公路"，LN 放在子层入口不阻塞这条主干。
- **边界**：顺序写反（先子层后 LN）就变回 post-norm，教程专门警告"顺序很重要"；本课只在 2 层小网络上验证趋势，pre/post 的深层差异要到很深的网络才显著。

**Q4："多头注意力里每个头都在学什么？"**

- **结论**：每个头是一条独立的低维通信通道，各自捕捉一种"话题"（找元音、找位置、找标点……），多头 = 并行多通道。
- **原理**：教程 02 章的表述是"一个头只有一个通信通道，这么多话题挤在 32 维通道里太挤"；实现上 `head_size = n_embd // n_head`，各头输出 `cat` 后再 `proj` 回 `n_embd`（类比分组卷积）。课程实测：原视频多头让 val loss 从 2.4 → 2.28，但 CPU 小规模下单头 ≈2.39 vs 多头 ≈2.45 基本同量级。
- **边界**：多头的收益要在大数据、深网络下才充分显现——面试时别拿小规模数字当成多头无用的证据，教程原话是"看趋势、看结构收益"。

**Q5："模型初始化完、还没训练，loss 应该是多少？"**

- **结论**：应接近均匀分布的熵 $\ln V$；tiny Shakespeare 词表 65，即 $\ln 65 \approx 4.17$。
- **原理**：随机初始化下每个词的预测概率接近均匀，交叉熵期望是 $\ln V$；课程实测 bigram 初始 loss ≈ 4.87（随机初始化偏离严格均匀），训练后 ≈ 2.5，一路压到 scale-up 后的 1.48（原视频数字）。这道 sanity check 能当场暴露初始化错误、数据错位或 loss 平均口径问题。
- **边界**：只在"输出接近均匀"的初始化下成立；词表大时 $\ln V$ 本身就大（50K 词表约 10.8），要和同口径的基线比，不能跨模型硬比。
