...
> 跑 [scripts/13_reasoning_models.py](../scripts/13_reasoning_models.py)。
...
**实测（GPU 档：5000 SFT + 300 GRPO，单位数加法）**：
```
SFT loss → 0.2177（模型学会了 think+answer 格式与正确答案）
```

> 📌 **口径与诚实声明**：0.2177 是 GPU 档实测；CPU 档复跑 SFT loss ≈ 0.68（训练步数
> 与设备不同，数字不可比、看趋势）。脚本中"推理 RL（GRPO 简化版）"阶段只落地了
> 奖励设计（规则准确率 + 0.5×格式分）的展示，**未跑完整 RL 训练循环**——想动手实现，
> 用 04 章 GRPO 公式 + [07_grpo_training.py](../scripts/07_grpo_training.py) 的完整实现。
...
```
n=1: 准确率 56%    ← 单次采样          （GPU 档实测）
n=4: 准确率 46%    ← 玩具模型波动（n=4 时众数不稳定）
n=8: 准确率 58%    ← 更多样本 → 众数趋近真实分布 → 恢复并超过 n=1
```

> 📌 口径：上表为 GPU 档实测；CPU 档复跑为 n=1/4/8 = 58%/70%/80%（单调上升）——
> 两档都支持"n↑ 众数更稳"的方向，但单次数字环境敏感，别死记。
...
无独立计分题——本脚本的 test-time compute 实验（n=1/4/8 的 self-consistency 对比）即作业：
自己跑一遍 `scripts/13_reasoning_models.py`，记录三种 n 下的准确率，并解释"n 大不一定更好"的原因。

## 下一步

多模态如何让模型"看懂"图片？→ Part 15 多模态理解（课程主线下一站是 Part 9 CUDA 内核，
见 [Part 8 README](README.md) 末尾导航）。

👉 [Part 15 多模态理解](../../Part15_vision_language/tutorial/README.md)

---

[← 上一章：LoRA 与分类微调](08_lora_and_classification.md) | [Part 8 README](README.md)
