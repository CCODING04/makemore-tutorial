...
|------|:---:|:---:|:---:|:---:|:---:|
| CPU 缩小版 | 64 | 4 | 2 | 256 | ~0.13M |
| ~1M | 128 | 4 | 4 | 1000 | ~1M |
| ~30M | 256 | 8 | 6 | 50304 | ~30M |
| ~90M | 512 | 8 | 12 | 50304 | ~89M |
| 标准 GPT-2 | 1024 | 16 | 24 | 50257 | ~405M |

> 📌 口径：按 **untied**（输入/输出嵌入不共享）计算，这正是本课脚本的实现；
> 公式未计位置嵌入与 LN 的少量参数（~1% 偏差，如 ~90M 档公式 89.3M / 实测 89.6M、
> 标准 GPT-2 档实测 406M、CPU 缩小版实测 0.14M）。若开 weight tying（共享嵌入），
> 每档可再减 `vocab × embed`（如 ~90M 档降至 ~64M）。

💡 本教程用 CPU 缩小版（~0.1M 参数）快速演示。GPU 模式用 ~90M 配置（embed=512, 12 层），可在单张 4090 上跑通全流程。原理完全一样，只是规模不同。
