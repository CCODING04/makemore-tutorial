...
📊 **交互动图（页内）**：奖励差如何变成选择概率——横轴是 `r(A) − r(B)`，**悬停**查看任意奖励差对应的概率（如差值 +2 → 概率 ≈ 0.88）：
...
```plot
{
  ...
  "x": [-7, 7], "y": [-0.12, 1.12],
  "curves": [
    {"expr": "1/(1+Math.exp(-x))", "label": "P(A>B) = σ(rA − rB)"}
  ],
  "hlines": [
    {"y": 0, "label": "y = 0"}, {"y": 1, "label": "y = 1"}
  ],
  "points": [[0, 0.5, "(0, 0.5)：奖励打平 → 五五开"], [2, 0.8808, "差 +2 → ≈0.88"]]
}
```
