...
| 成本 | 每 chunk 一次 LLM 调用（Anthropic 用 prompt caching 把 248M chunk 的成本压到约 1.02 美元/百万文档 token，官方口径） | 零 LLM 调用；但要求嵌入模型支持长上下文 + 输出逐 token 向量 |
...
可复制运行命令（在仓库根目录；详见 [README 环境块](README.md)）：

```bash
cd courses/Part18_rag/scripts
# 主模式（模型已在本地缓存时先设离线开关，逐位复现下方表格；RTX 4090 实测 49-54s）
HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=0 python 02_contextual_retrieval.py
# 降级模式（零模型、纯 CPU，~3s，输出与下方 ⚠️ 降级留档一致）
RAG18_FORCE_FALLBACK=1 python 02_contextual_retrieval.py
# 评测脚本（03）同款两档：
HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=0 python 03_rag_eval.py     # ~10-17s
RAG18_FORCE_FALLBACK=1 python 03_rag_eval.py                      # ~1s
```

> 📊 环境标注：与 01 章相同（RTX 4090 / torch 2.6.0+cu124 / transformers 4.57.6）；
> 定位句由 Qwen2.5-0.5B-Instruct 贪心生成（生成上限 64 token、典型输出 ~50，
> 输入=文档大纲 + chunk 前 350 字）；238 chunk 全量。**确定性声明**：生成与裁判
> 全程贪心解码、结构前缀为确定性规则——本表不依赖随机种子（seed 不适用），
> 同环境重跑应逐位复现。耗时三档：模型已缓存（`HF_HUB_OFFLINE=1`）50-55s
> （实测 49-54s，其中 LLM 定位句 ~28-32s）；模型缺失直降级 ~3s；模型在缓存但 HF
> 在线校验不通（代理坏/弱网）→ 每模型重试耗尽后才降级，实测 143-157s 且数字变成
> 下方降级留档——先试 `HF_HUB_OFFLINE=1`（共享 GPU 上均有波动）。
...
[Step 2] 实验一：plain → +LLM 前缀 → +BM25 混合(RRF k=60) → +rerank
  ...
  失败率                                      0.00  0.00  0.00  0.00
        （官方: 5.7% / 3.7% / 2.9% / 1.9%，累计 -67%；
          口径=失败率，与本机 recall@20 不可直接比——见归因第 3 条）
...
![实验一四阶梯与实验二格式噪声对比（本机实测 vs 官方口径）](../images/contextual_retrieval_ablation.png)

> 🖼 上图：左图为实验一四阶梯 mean（主模式 vs 降级留档；本机 recall@20 与官方
> 失败率分属两个口径，仅示意"方向 vs 幅度"，不可逐点比大小）；右图为实验二
> "同信息换排版"的逐查询 recall 摆动（±0.08 与任何单级增益同量级）。逐格数值
> 以上方输出块与下表为准。

| 实验 | 阶段/排版 | Q1 | Q2 | Q3 | mean |
|---|---|---|---|---|---|
| 实验一 | plain / +LLM / +BM25 / +rerank | 0.75 0.75 0.75 0.75 | 0.58 0.42 0.50 0.50 | 0.67 0.67 0.58 0.58 | **0.67 0.61 0.61 0.61** |
| 实验二 | plain / A / B / A&LLM | 0.75 1.00 0.75 1.00 | 0.58 0.50 0.50 0.33 | 0.67 0.67 0.67 0.67 | **0.67 0.72 0.64 0.67** |
| 官方 | 失败率（不可比口径） | — | — | — | 5.7% → 3.7% → 2.9% → 1.9% |

> ⚠️ **降级模式实测**（`RAG18_FORCE_FALLBACK=1`）：hashing 向量 + 空定位句下
> 四阶梯 mean 仅 0.11→0.11→0.17→0.17、失败率 33%——连 top-20 都摸不到大部分相关
> chunk。上下文工程救不了烂嵌入。
>
> ⚠️ **网络前提再强调**：模型已在本地缓存时（`huggingface-cli scan-cache` 可查），
> 一定要 `HF_HUB_OFFLINE=1`——否则代理坏/弱网时脚本会在重试后"静默"降级，你拿到的
> 就是上面这组降级数字，还以为教程错了。降级运行下脚本 03 的 context_precision
> 会**三问全 0.00**（关键词裁判解析不了相关性判定，比 faithfulness 的 1.00→1.00
> 更极端）——同为"降级裁判零判别力"的证据，教程在此一并预告。
...
>
> 📝 **论文数字声明**：本节引用的 LaRA / Self-Route / context engineering 数字
> （17%、39%、65% 等）均为**论文原文数字，本环境不可复现**（需要其私有基准与
> 商用模型），引用时请注明出处并按"量级"而非"精确值"使用。
...
查询路由给全文模式——效果接近纯长上下文，**成本比纯长上下文降 39%（GPT-4o）
~65%（Gemini-1.5-Pro）**（论文原句口径："the cost is reduced by 65% for
Gemini-1.5-Pro and 39% for GPT-4o"）。换算写明：降幅 39%~65% 与"Self-Route
开销占纯长上下文的 **35%~61%**"（=1−65% ~ 1−39%）互为补数，是同一测量的两种说法；
该开销仍显著高于纯 RAG 的约 17%——因为绝大多数查询被路由留在便宜的 RAG 档，
只有少数才升级到长上下文。注意两端点是**按模型给出**的（GPT-4o vs Gemini），
不是一个连续区间，引用时按量级使用。
...
| **Context Precision**（上下文精确率） | 检索回的 contexts 逐条判"对回答有用吗"（脚本实现问的是"**相关吗**"，缘由见下方读数 2），按 AP 口径聚合 | 噪声（检索塞没塞无关材料） |
...
> "幻觉版" = 在正确答案后拼两句无中生有的话。总耗时三档：模型已缓存
> （`HF_HUB_OFFLINE=1`）实测 10-12s；模型缺失直降级 ~1s；模型在缓存但 HF 在线
> 校验不通 → 重试耗尽后降级（关键词裁判），实测 188-205s。
...
[Step 2] faithfulness：grounded vs 拼接幻觉句
  ...
  （"拉低 0.15"按未舍入均值差 0.1533 计算，显示值相减为 0.16——口径注）
...
**AP 口径 30 秒手算**（上面的 Q1 全 1 太平凡——AP 必为 1.00，验证不了公式；
换一个非平凡判定向量 [1, 0, 1, 1, 0] 自己算一遍）：
precision@k = 前 k 位中相关数 / k，逐位得 P 序列 1/1, 1/2, 2/3, 3/4, 3/5；
AP = **相关位次上**的 P 值平均（第 2、5 位无关，不计入）= (1 + 2/3 + 3/4) / 3
= (29/12) / 3 = 29/36 ≈ **0.81**。手算检验点：AP 只在相关位次"取值"，
但无关位次仍通过分母 k 惩罚"排在前面"的相关项——把三个相关项整体后移到
[0, 0, 1, 1, 1]，AP 立刻从 0.81 掉到 (1/3 + 1/2 + 3/5)/3 = 43/90 ≈ 0.48。
（Kendall τ 一行直觉：裁判给的相关性排名 vs 检索器排名里，同序对与逆序对
之差 ÷ 总对数，+1 完全一致、−1 完全相反；标签全 1 或全 0 时无区分度，记 n/a。）
...
3. **评测器噪声是结构性的**：同一陈述、等价问法，判决在 yes/no 之间翻转；
   ...
   + 只做**系统间相对比较**（A/B 谁高谁低可信，绝对值 0.56 vs 0.40 不可信）
...
一种）时，context precision = **相关位次上 precision@k 的平均（AP 口径，
手算例见上文 Step 3 之后的"30 秒手算"）**，context recall = ground truth
证据被检索列表覆盖的比例——纯确定性计算。
而 faithfulness / answer relevancy 本质是语义判断（"这句话算不算被支持"），
规则替代的误差大（脚本 03 降级模式的关键词裁判就抓不出幻觉句，实测
grounded=1.00、+幻觉=1.00，context_precision 更是三问全 0.00——降级运行
留档可复现）。所以生产上常见组合：离线回归用确定性指标，
抽样审计用 LLM 裁判。
</details>
...
> 📝 课程路线是"从零手写"，下面四个工业界工具**本课程未实测**，定位均为
> 社区/官方口径（星数量级为 2025 下旬的社区常见引用），供选型时对号入座——
> 手写件的每一层都能在其中找到产品化对应物。

| 工具 | 一句话定位 | 与本课程手写件的关系 |
|---|---|---|
| **RAGFlow**（~89.6k★） | 平台化 RAG 引擎：知识库管理、深度文档解析（表格/版面）、引用溯源开箱即用 | 把 01-02 章整条管线产品化；"深度文档理解"正对本章"chunk 断语境"问题的工程化答案 |
| **LlamaIndex**（~51.9k★） | RAG 数据框架：索引/检索器/查询引擎分层抽象，原型到生产换件方便 | 其 `Index` ≈ 我们的向量矩阵 + 分块；`Retriever` ≈ dense/BM25/RRF；换嵌入模型零改业务码 |
| **Data-Juicer**（阿里，~10k+★） | 面向 LLM 的数据处理系统：清洗/去重/过滤 50+ 算子 | 管"语料进向量库之前"那一段（Part 13 思想的工程化），RAG 语料质量的第一道闸 |
| **vLLM**（→ [Part 14](../../Part14_inference_vllm/tutorial/README.md)） | 高吞吐推理引擎：PagedAttention/连续批处理 | RAG 生成侧与"裁判侧"的生产底座；长上下文成本账（§三）在它上边才算得准 |

一句收束：**手写为了懂原理，选型为了交工程**——面试里"你的手写件和
RAGFlow/LlamaIndex 比差什么"的标准答法：差工程化（文档解析/权限/编排/持久化），
不差原理；差的那些正是它们收走抽象税换来的。

## 课后作业

👉 [Assignment 18](../../../assignments/assignment_18/) 中，**题 4
（faithfulness：judge 可注入设计）与 ragas 实验题**与本章 §四 直接对应——
做完手写题再回来跑 `RAG18_FORCE_FALLBACK=1 python 03_rag_eval.py` 对拍降级
裁判的 1.00→1.00，体会"评测器本身是被测对象"；思考题 Q2（unsure 口径）
是脚本 03 `_parse_verdict` 保守计 0 的镜像。
