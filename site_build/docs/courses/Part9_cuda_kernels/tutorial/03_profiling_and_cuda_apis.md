...
> 另注意 Memory 与 Compute (SM) 两个主指标都是 92.01%——这不是誊写错误，复跑同样
> 出现（91.60 = 91.60）：该内核两条指标被**同一条 L1/TEX 管线**顶满（L1/TEX 94.15%
> 才是第一瓶颈）。SOL 表读的是"哪条管线先满"，主指标同值时要下钻看子行
> （L1/TEX vs DRAM）才能分清瓶颈。
...
**第一步：一块"行主序存储"的缓冲区，按列主序去读，读到的正好是它的转置。**
用 $2 \times 2$ 的例子走一遍内存布局。矩阵 $A = [[1, 2], [3, 4]]$ 在内存里按行摊平成
`[1, 2, 3, 4]`；若把这 4 个数按**列主序**解读（每列 2 个数、第 1 列排完再排第 2 列），
还原出来的方阵是 `[[1, 3], [2, 4]]`——恰好是 $A^{\top}$（行变列、列变行）。

**第二步：代数恒等式** $(A@B)^{\top} = B^{\top}@A^{\top}$（转置会颠倒乘法顺序）。
要算行主序 $C = A@B$：三块缓冲区在列主序视角下"读出来"分别是 $A^{\top}$、$B^{\top}$、
$C^{\top}$，而

$$C^{\top} = (A@B)^{\top} = B^{\top}@A^{\top}$$

→ 让 cuBLAS 算 "$B^{\top}@A^{\top}$"（指针换序，不搬运任何数据），写回的内存就是行主序 $C$：
