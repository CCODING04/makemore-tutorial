...
| **对数 (log)** | 把乘法变成加法的工具，$\log(a \times b) = \log(a) + \log(b)$ |
| **似然 (likelihood)** | 给定模型，观察到数据的概率 |
| **自然对数** | 底数为 e 的对数，PyTorch 里 `torch.log()` 就是自然对数 |

> 🔑 重点记住：**概率相乘 → 取 log → 相加 → 取负**，这就是后面 NLL Loss 的推导链条。这条链条还会给出一个重要的锚点数字：一个"完全瞎猜"的模型，loss 恰好是 $\ln 27 \approx 3.30$——后面学到 NLL 时会解释它从哪来。
...
数据集规模大约 **32,000 个名字**（精确计数 32,033 个），共包含约 **19.6 万个字符**（196,113 个）。把每个名字首尾加上起止符 `.` 后，相邻字符对（也就是下一节要统计的 **bigram**）共有 **228,146** 个——这三个数字后面会反复用到，别混用。字符集是 26 个小写字母加上特殊的开始/结束标记 `.`。
