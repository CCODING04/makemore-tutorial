...
分叉（按赛道）             算法赛道：缺口登记 → 补 scaling law / 评估实操 / 多模态
                           应用赛道：应用线 A1 最小 RAG → A2 Agent（已建成 Part 18/19，姊妹系列）
```
...
> （实战层）。v3 缺口登记的 7 个缺口也已于 v3.1 全部闭环，见
> [§6 缺口登记](#6-缺口登记v31-已全部闭环)。
...
> | 预训练岗 | 节点 13 → 11 → 12 → 14（scaling law 已建 Part 13 00 章，从它入门） |
...
> 状态：**已建成**（v3.1）——载体为 Part 18（RAG）/ Part 19（Agent），课程内
> "先手写、后 harness"，与算法主线同一套 tutorial-creator 质量门禁（≥4.5）。

#### 应用线 A1 · 最小 RAG（≈8-15h，✅ Part 18）

- **学习内容安排**（已落地 `courses/Part18_rag/`）：手写五件套（递归分块 / embedding /
  BM25 / RRF 混合 / 重排 → 生成）→ contextual retrieval 复刻 → 手写 faithfulness /
  context precision 评测 → 对比 ragflow（89.6k★）平台化能力与 llama_index（51.9k★）抽象。
- **学习目标设定**：能讲 RAG 全链路每步的失败模式（chunk 策略与召回率、混合检索、
  幻觉抑制）；能回答"什么时候不该用 RAG"。
- **学习验证**：跑通脚本 01-03（recall@5 逐级提升的对比表）+ 作业 18（4+1）；
  链 6（应用链）前半段自问自答。

#### 应用线 A2 · Agent（≈8-15h，✅ Part 19）

- **学习内容安排**（已落地 `courses/Part19_agents/`）：手写 agent loop（模型 + 工具表 +
  while 循环 + 三终止条件 + 沙箱白名单）→ mini-MCP（JSON-RPC 2.0 三方法）→ τ-bench
  微缩（政策 + 用户模拟器 + pass^1）→ 框架认知（LangGraph / OpenAI Agents SDK /
  smolagents / 极简派 Pi）。
- **学习目标设定**：能手写最小 function calling 循环并说清中断/重试/上下文管理三件事；
  能评价各框架的抽象收益与成本。
- **学习验证**：跑通脚本 01-03（含一次失败恢复轨迹）+ 作业 19（4+1）；
  链 6 后半段自问自答。
...
- **预训练岗**：主线节点 7/9/10 是重心；scaling law 从 Part 13 00 章入门 + 节点 13 提前。
...
> 补齐；v3.1（2026-09-01）把下表 7 个可行动缺口**全部落地**（多模态维持归档）：

| 缺口 | 所属簇 | 落地去向 | 状态 |
|---|---|---|---|
| RAG 全链路（算法侧认知） | D | **Part 18**（手写五件套/contextual retrieval/评测 + 作业 18） | ✅ 已建（v3.1） |
| Agent / function calling | E | **Part 19**（手写 agent loop/mini-MCP/τ-bench 微缩 + 作业 19） | ✅ 已建（v3.1） |
| scaling law | G | **Part 13 00 章**（Chinchilla 拟合/isoFLOP/epoch 三实验） | ✅ 已建（v3.1） |
| 幻觉 / 安全对齐专题 | H | **Part 8 07 章** §6-§8（SE/温度 sweep/ECE 三实验 + 合规一页纸） | ✅ 已扩（v3.1） |
| lm-eval-harness 实操 | H | **Part 8 脚本 12**（arc_easy fewshot 对比 + 自定义 task） | ✅ 已扩（v3.1） |
| 长上下文 YaRN 实验 | A | **Part 7 脚本 11 加 yarn + 脚本 13 迷你 RULER** | ✅ 已扩（v3.1） |
| Flash Attention 手写内核 | F | **Part 9 脚本 09 + 05 章**（Triton 内核 ≥ SDPA 最优后端 105%） | ✅ 已扩（v3.1） |
| 多模态 VLM | A 补充 | Part 15（可选，LLaVA / Qwen-VL 系） | 归档（v2-T9） |

## 修订记录

- **v3.1（2026-09-01）**：缺口补全计划（`docs/gap_closure_plan.md`）9 任务全部落地：
  新建 Part 18（RAG）/ Part 19（Agent）+ 作业 18/19，Part 7 加 YaRN + 迷你 RULER，
  Part 8 扩幻觉/安全/合规 + lm-eval 实操，Part 9 新增手写 Triton FA 内核章，
  Part 13 新增 scaling law 开篇章；§6 缺口登记 7 行全部翻转闭环。
