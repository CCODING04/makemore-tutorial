...
- **03/04 章**：DPO/PPO/GRPO 概念——§6.4 校准对比和 §7 安全对齐直接建立在其上
...
1. ⚠️ **data_files 相对路径以"运行时的 cwd"为基准解析**，不是 yaml 所在目录——
   从项目根跑 `--include_path mytasks/` 会找不到 jsonl。脚本 12 的做法：**启动时用基于
   `__file__` 的绝对路径重写 yaml** 再交给 TaskManager，从任何目录跑都成立。
...
📝 注意这是 GPT-4 时代（RLHF/PPO 系）的结论；DPO 系算法的最新证据见 TruthRL（2509.25760）。
...
> （95% 对 vs 78% 置信），Instruct 过自信（65% 对 vs 79% 置信），"自信没降、正确率大幅
> 掉"即"自信掉了、正确率也掉了"的校准恶化现场。0.5B + 20 题样本量，**看方法、看方向，
> 别看效应量**。

### 6.5 缓解的工程正道

1. **RAG**：生成前检索外部依据拼进上下文，把"闭卷考试"变"开卷考试"——治"知识缺失"型幻觉（本课 09 章推理模型之外的知识侧方案）。
...
DPO 系是否有同样代价尚在研究（TruthRL 2509.25760）。
</details>
...
📝 论文还提出 **causal completion**：把"方向"当因果变量，以"消融后拒绝行为恢复程度"
为优化目标直接求解方向——比 diff-in-means 更准也更贵，diff-in-means 是性价比之选。
...
| **大模型（生成式 AI）备案/登记** | 《生成式 AI 服务管理暂行办法》第 17 条 | 基于已备案模型提供服务的，登记申明即可；自研模型走安全评估 |
...
> tokenizer/采样知识同源。今天是 2026-09-01——标识办法施行一周年，已是从业常识。
