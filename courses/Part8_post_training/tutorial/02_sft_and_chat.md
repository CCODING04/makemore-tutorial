...
实际效果：masked loss **通常**比 unmasked loss **更大**——因为只看"难的部分"（回答），不看"容易的部分"（复制 prompt）。⚠️ 但这是经验规律不是数学保证：欠训练模型 + 合成数据时两者可能接近甚至相反（本课脚本实跑就出现过 unmasked 2.7361 vs masked 2.7182）——masking 的意义在"只学回答"这个**训练目标正确性**，不在"loss 一定更大"，看趋势别死抠单次数字。
