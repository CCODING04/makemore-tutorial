...
- ✅ **推导** ZeRO 三阶段的每卡公式：$4\Psi+12\Psi/N \to 2\Psi+14\Psi/N \to 16\Psi/N$，及各自的通信代价
...
| 模型 | DDP（每卡 $16\Psi$） | ZeRO-1（N=8） | ZeRO-3（N=8） |
|---|---:|---:|---:|
| 7B | 112 GB | 38.5 GB | 14 GB/卡 |
| 70B | 1120 GB | 385 GB | 140 GB/卡 |

![ZeRO 三阶段每卡模型状态对比（7B、N=8，公式值，1 GB = 1e9 字节，不含激活）](../images/zero_memory_bar.png)

- 🔑 **这就是"为什么 7B 在 24G 卡上训不动"的完整答案**：哪怕 bf16 存参数只要 14GB，
  AdamW 状态一加就到 112GB。ZeRO 的全部思想就一句话：**这 $16\Psi$ 里大部分是优化器状态，
  没必要每个卡都存完整份**。

**ZeRO 三阶段**（Rajbhandari et al. 2019，[arXiv 1910.02054](https://arxiv.org/abs/1910.02054)）：

| 阶段 | 切什么 | 每卡模型状态 | 通信代价 |
|---|---|---:|---|
| ZeRO-0 (DDP) | 什么都不切 | $16\Psi$ | 基准 |
| ZeRO-1 | 优化器状态 | $4\Psi + 12\Psi/N$ | 与 DDP 相同 |
| ZeRO-2 | +梯度 | $2\Psi + 14\Psi/N$ | 与 DDP 相同 |
| ZeRO-3 / FSDP | +参数 | $16\Psi/N$ | ≈1.5× DDP |

[脚本 03](../scripts/03_zero_memory.py) Part B 把每个 rank 持有的每一类张量逐字节加起来，
断言 ZeRO-1 与公式 $4\Psi+12\Psi/N$ 一致（ZeRO-2/3 的逐字节复算留作本章动手 2）——面试被追问
"12Ψ/N 怎么来的"时，这就是你的证据链。

- ⚠️ **激活值不在这本账里**：激活约 $s \cdot b \cdot h \times \left(34 + 5 \cdot a \cdot s / h\right)$ 字节/层
  （$s$=seq, $b$=batch, $h$=hidden, $a$=heads；出处 Korthikanti et al. 2022,
  [arXiv 2205.05198](https://arxiv.org/abs/2205.05198)，与并行方式无关、随 seq² 有平方项）。
  省激活靠**梯度检查点**（只存层输入、反传时重算，显存从每层全存降到 $2sbhL$，
  代价 ≈ 多一次 forward，即 +33% 计算）。
...
[脚本 04](../scripts/04_fsdp_gpt.py)（**需要 GPU**：FSDP1 在 torch 2.x 不支持 CPU 加速器，
纯 CPU 机器请改跑脚本 03 的记账模拟；双卡实测，torch 2.6.0+cu124, NCCL）：
...
（单卡跑也能通过，但 world=1 退化为 NO_SHARD：显存读数是 12.7 MB / loss 3.005——
**这些数字随 world_size 变**，对照分片效果请用 ≥2 卡。）
...
① 激活（seq/batch 大时轻松几十 GB → 梯度检查点）；

② 通信是否成为瓶颈（ZeRO-3 的 1.5×）；

③ NCCL 通信缓冲与碎片余量（实际可用 <24GB）。
...
- [ ] N=8 时你的表与教程 §1 的表对得上，并补齐它没列的 ZeRO-2 一档：7B：DDP 112 /
      ...
      （GB 按 1e9 字节口径，与脚本一致）
- [ ] 任意 N>1 都有 ZeRO-2 < ZeRO-1：两式相减 $(2\Psi+14\Psi/N) - (4\Psi+12\Psi/N) = (2/N-2)\,\Psi < 0$，
...
**验收标准：**
- [ ] stage2：`sum(per_rank)/N` 与公式 $2\Psi+14\Psi/N$ **严格相等**（分片不重不漏 → 平均值精确
      对上；对照公式别用 `==`，用 `abs(a-b) < 1e-6`）
- [ ] 单个 rank 与公式的偏差不为 0 的情形：N=3 时能看到百字节量级的 rank 间差
      （本机实测 stage2 @ N=3：+233 / −103 / −131 字节）——"公式是均摊值"的直观含义
- [ ] stage3：`sum(per_rank)/N == 16*Ψ/N`，且 N=1 时 stage 0/1/2/3 四条全部退化为 $16\Psi$
      （assignment 题 2 的"自检锚点"同款）
- [ ] 用脚本里的 TinyGPT（$\Psi$=6,298,624）复核一个数：stage3 @ N=2 每卡 ≈ **50.4 MB**
      （$16\Psi/2$；对照 Part B 打印的 $16\Psi$ = 100.8 MB）
