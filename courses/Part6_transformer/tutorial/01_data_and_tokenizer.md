...
关键洞察：一块 9 个字符的连续序列，其实包含了 **8 个训练样本**。因为每个位置都对应一个"用前 t 个字符预测第 t+1 个字符"的样本。[02_bigram_baseline.py](../scripts/02_bigram_baseline.py) 里我们用 `train_data[:block_size + 1]` 实际打印出来（✦ 标记每行**新增**的输入 token）：

```chunk
{
  "title": "一个 chunk 内含多个样本",
  "subtitle": "取前 8+1 = 9 个字符，按顺序偏移形成 8 个训练样本",
