...
目标：在单头 Self-Attention 基础上分三步叠加组件，观察 val loss 逐级变化：
  Phase 1 多头并行（Multi-Head）        → ≈2.45（与单头 ~2.4 在同一量级）
  Phase 2 + 前馈网络（FeedForward）      → 少步数下可能略升（收敛变慢），多跑步数会降
  Phase 3 + 残差连接（Residual）         → ≈2.23（梯度超高速公路，下降最明显）
...
═══ 演进对比 ═══
  ...
  Phase 2 + 前馈网络:      {v2:.4f}
  Phase 3 + 残差连接:      {v3:.4f}
  多头让多个通信通道并行 → val loss ≈2.45（与单头 ~2.4 在同一量级，少量步数下多头优势不明显）；
